...
> **문서 종류 및 네임스페이스 주의.** 이 글은 정식 논문이 아닌 **에세이**(서사 형식)라서 `paper/essays/` 하위로 옮겨져 있다. 본문의 "L1–L9 (+L10)"는 **HAL9 소프트웨어 아키텍처 계층 스택**(반사 → 실행 → 기능 → 아키텍처 → 엔터프라이즈 → 철학 → 형식 패턴 → 메타-계층 → 의식 → 통합)을 가리킨다. HAL 논문 시리즈의 L1–L5(수면-각성, 에이전트-신경망 동형성, 7±2 분기, 에너지 스케일링, 장이론 — `paper/HAL/` 아래)와는 **다른 번호 체계**다. 접두어가 같다고 섞어 읽지 말 것.
